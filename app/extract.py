...
while numregistros <= tope:
    ...
    if numregistros==0:
        ...
        page = requests.post(url,headers=headers,params=params, data=data)
        #input('primer quiebre')
        #print(page.text)
        #input('segundo quiebre')
        time.sleep(10)
        dataWeb = page.json()
        time.sleep(10)
        #input('tercer quiebre')
...
dfDataWeb['en_revision'] = 'false'
dfDataWeb['nombre_revisor'] = ''
...
def load_to_db(df,connstr,tbname):
    ...
    #esto para otro momento en el código
    #cursor = conn.cursor()
    
    #sql1 = '''select * from ''' + tbname + ''' limit 1;'''
    #cursor.execute(sql1)
...
from databaselib import MergeWebScrappingData as mw # Assuming your module is named merge_web_scrapping_data.py
conn_string = 'postgresql://postgres:imaiden123@localhost:5432/infy_projects'

def update_web_scrapping_data(connstr,procedure_name):
    merge_processor = mw.MergeWebScrappingData(connstr,procedure_name)
    try:
        ...
        print(f"Error updating web scrapping data: {e}")
    finally:
        # Disconnect from the database
        merge_processor.disconnect()

# Call the function to update web scrapping data
update_web_scrapping_data(conn_string,'merge_web_scrapping_data')


## leer los registros que estan en curso para el día de hoy
## para generar una notificacion a los usuarios
